...
def create_aae(n_features, h_dim, z_dim):
    ...
    """
    encoder = AAEEncoder(h_dim, z_dim)
    decoder = AAEDecoder(n_features, h_dim)
...
def create_aae_scheduler(aae_optimizer, encoder_lr_scheduler=None, generator_lr_scheduler=None,
    ...
    lr_callback = MultiOptimizerLearningRateScheduler(zip(lr_schedules, optimizers))
    return lr_callback
